...
- **Out of scope for the admin (still hardcoded)**: `Testimonials` — the `testimonials` table exists if this is revisited later, but no admin screen exists yet. `Stats` is editable at `/admin/stats` (`stats` table, via `ListEditor`).
